describe('Liquid', () => {
  ...
  if (baseModule === 'liquid') {
    ...
    it('loads a specific block page', () => {
      cy.visit(
        `${basePath}/block/7e1369a23a5ab861e7bdede2aadcccae4ea873ffd9caf11c7c5541eb5bcdff54`
      );
    ...
    describe('peg in/peg out', () => {
      it('loads peg in addresses', () => {
        cy.visit(
          `${basePath}/tx/fe764f7bedfc2a37b29d9c8aef67d64a57d253a6b11c5a55555cfd5826483a58`
        );
        cy.waitForSkeletonGone();
        ...
        cy.get('.table-tx-vin a')
          ...
          .then(() => {
            ...
            if (baseModule === 'liquid') {
              ...
              cy.url().should(
                'eq',
                'http://localhost:4200/tx/f148c0d854db4174ea420655235f910543f0ec3680566dcfdf84fb0a1697b592'
              );
            }
          });
      });
      ...
      it('loads peg out addresses', () => {
        cy.visit(
          `${basePath}/tx/ecf6eba04ffb3946faa172343c87162df76f1a57b07b0d6dc6ad956b13376dc8`
        );
        cy.waitForSkeletonGone();
        //Remove the target=_blank attribute so the new url opens in the same tab
        cy.get('.table-tx-vout a')
          .first()
          .invoke('removeAttr', 'target')
          .click()
          .then(() => {
            ...
            if (baseModule === 'liquid') {
              ...
              cy.url().should(
                'eq',
                'http://localhost:4200/address/1BxoGcMg14oaH3CwHD2hF4gU9VcfgX5yoR'
              );
            }
    ...
    describe('assets', () => {
      ...
      it('shows a specific asset ID', () => {
        ...
        cy.get('.container-xl input')
          ...
          .type('Liquid AUD')
          .then(() => {
            cy.get('ngb-typeahead-window:nth-of-type(1) button', {
              timeout: 30000,
            }).click();
          });
      });
    ...
    describe('unblinded TX', () => {
      it('should not show an unblinding error message for regular txs', () => {
        cy.visit(
          `${basePath}/tx/82a479043ec3841e0d3f829afc8df4f0e2bbd675a13f013ea611b2fde0027d45`
        );
        cy.waitForSkeletonGone();
      ...
      it('show empty unblinded TX', () => {
        cy.visit(
          `${basePath}/tx/f2f41c0850e8e7e3f1af233161fd596662e67c11ef10ed15943884186fbb7f46#blinded=`
        );
        cy.waitForSkeletonGone();
        cy.get('.table-tx-vin tr:nth-child(1)').should(
          'have.class',
          'ng-star-inserted'
        );
        cy.get('.table-tx-vin tr:nth-child(1) .amount').should(
        ...
        cy.get('.table-tx-vout tr:nth-child(1)').should(
          'have.class',
          'ng-star-inserted'
        );
        cy.get('.table-tx-vout tr:nth-child(2)').should(
          'have.class',
          'ng-star-inserted'
        );
        cy.get('.table-tx-vout tr:nth-child(1) .amount').should(
      ...
      it('show invalid unblinded TX hex', () => {
        cy.visit(
          `${basePath}/tx/f2f41c0850e8e7e3f1af233161fd596662e67c11ef10ed15943884186fbb7f46#blinded=123`
        );
        cy.waitForSkeletonGone();
        cy.get('.table-tx-vin tr').should('have.class', 'ng-star-inserted');
        cy.get('.table-tx-vout tr').should('have.class', 'ng-star-inserted');
        cy.get('.error-unblinded').contains(
          'Error: Invalid blinding data (invalid hex)'
        );
      });

      it('show first unblinded vout', () => {
        cy.visit(
          `${basePath}/tx/f2f41c0850e8e7e3f1af233161fd596662e67c11ef10ed15943884186fbb7f46#blinded=100000,6f0279e9ed041c3d710a9f57d0c02928416460c4b722ae3457a11eec381c526d,0ab9f70650f16b1db8dfada05237f7d0d65191c3a13183da8a2ddddfbde9a2ad,fd98b2edc5530d76acd553f206a431f4c1fab27e10e290ad719582af878e98fc`
        );
        cy.waitForSkeletonGone();
        cy.get('.table-tx-vout tr:nth-child(1)').should(
          'have.class',
      ...
      it('show second unblinded vout', () => {
        cy.visit(
          `${basePath}/tx/f2f41c0850e8e7e3f1af233161fd596662e67c11ef10ed15943884186fbb7f46#blinded=2364760,6f0279e9ed041c3d710a9f57d0c02928416460c4b722ae3457a11eec381c526d,90c7a43b15b905bca045ca42a01271cfe71d2efe3133f4197792c24505cb32ed,12eb5959d9293b8842e7dd8bc9aa9639fd3fd031c5de3ba911adeca94eb57a3a`
        );
        cy.get('.table-tx-vout tr:nth-child(2').should(
          'have.class',
          'assetBox'
      ...
      it('shows asset peg in/out and burn transactions', () => {
        cy.visit(
          `${basePath}/assets/asset/6f0279e9ed041c3d710a9f57d0c02928416460c4b722ae3457a11eec381c526d`
        );
        cy.waitForSkeletonGone();
      ...
      it('prevents regressing issue #644', () => {
        cy.visit(
          `${basePath}/tx/393b890966f305e7c440fcfb12a13f51a7a9011cc59ff5f14f6f93214261bd82`
        );
        cy.waitForSkeletonGone();
      });
    });
