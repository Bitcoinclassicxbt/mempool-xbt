...
export class BlockPreviewComponent implements OnInit, OnDestroy {
    ...
    const block$ = this.route.paramMap.pipe(
      ...
      tap((block: BlockExtended) => {
        ...
        if( this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet' ) {
          ...
          this.seoService.setDescription($localize`:@@meta.description.bitcoin.block:See size, weight, fee range, included transactions, audit (expected v actual), and more for Luckycoin${seoDescriptionNetwork(this.stateService.network)} block ${block.height}:BLOCK_HEIGHT: (${block.id}:BLOCK_ID:).`);
        }
  ...
  // used anymore on Luckycoin networks (we use block.extras directly)
  setBlockSubsidy() {
    this.blockSubsidy = 0;
  }
