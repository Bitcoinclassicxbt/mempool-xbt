...
export class SearchFormComponent implements OnInit {
    ...
    this.typeAhead$ = combineLatest(
      [
        ...
        map((latestData) => {
          ...
          const isNetworkLuckycoin = this.network === '' || this.network === 'testnet' || this.network === 'testnet4' || this.network === 'signet';

          const matchesBlockHeight = this.regexBlockheight.test(searchText) && parseInt(searchText) <= this.stateService.latestBlockHeight;
          const matchesDateTime = this.regexDate.test(searchText) && new Date(searchText).toString() !== 'Invalid Date' && new Date(searchText).getTime() <= Date.now() && isNetworkLuckycoin;
          const matchesUnixTimestamp = this.regexUnixTimestamp.test(searchText) && parseInt(searchText) <= Math.floor(Date.now() / 1000) && isNetworkLuckycoin;
  ...
  selectedResult(result: any): void {
    if (typeof result === 'string') {
      ...
      this.navigate('/address/', result.searchText);
