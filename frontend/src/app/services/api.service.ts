...
export class ApiService {
  ...
  getHolders$(page?: number): Observable<IHolderApiResponse> {
    return this.httpClient.get<IHolderApiResponse>(
      this.apiBaseUrl + '/api/v1/holders?page=' + (page || 1)
    );
  }
  ...
  getEnterpriseInfo$(name: string): Observable<any> {
    return this.httpClient.get<any>(
      this.apiBaseUrl +
        this.apiBasePath +
        `/api/v1/services/enterprise/info/` +
        name
    );
  }
