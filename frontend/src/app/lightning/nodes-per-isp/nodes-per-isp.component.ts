    ...
    this.nodes$ = this.apiService.getNodeForISP$(this.route.snapshot.params.isp)
      .pipe(
        ...
        map(response => {
          ...
          this.seoService.setDescription($localize`:@@meta.description.lightning.nodes-isp:Browse all XBT Lightning nodes using the ${response.isp} [AS${this.route.snapshot.params.isp}] ISP and see aggregate stats like total number of nodes, total capacity, and more for the ISP.`);
