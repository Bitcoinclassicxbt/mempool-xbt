...
class BitcoinBackendRoutes {
  ...
  private static handleException(e: any, fnName: string, res: Response): void {
    if (typeof e.code === 'number') {
      ...
      const err = `exception in ${fnName}. ${e}. Details: ${JSON.stringify(e, [
        'code',
        'message',
      ])}`;
  ...
  private async $getMempoolEntry(req: Request, res: Response): Promise<void> {
    ...
    try {
      if (typeof txid !== 'string' || txid.length !== 64) {
        res
          .status(400)
          .send(`invalid param txid ${txid}. must be a string of 64 char`);
        return;
      }
    ...
    try {
      ...
      if (typeof verbose !== 'string') {
        res
          .status(400)
          .send(
            `invalid param verbose ${verbose}. must be a string representing an integer`
          );
        return;
      }
      ...
      if (typeof verboseNumber !== 'number') {
        res
          .status(400)
          .send(`invalid param verbose ${verbose}. must be a valid integer`);
        return;
      }
    ...
    try {
      if (typeof rawTxs !== 'object') {
        res
          .status(400)
          .send(
            `invalid param rawTxs ${JSON.stringify(
              rawTxs
      ...
      if (typeof txHex !== 'object' || txHex.length === 0) {
        res
          .status(400)
          .send(
            `testmempoolaccept failed for raw txs ${JSON.stringify(
              rawTxs
            )}, got an empty result`
          );
        return;
    ...
    try {
      ...
      if (
        ...
        res
          .status(400)
          .send(
            `invalid param verbose ${verbose}. must be a string ('true' | 'false')`
          );
        return;
      }
      ...
      if (!ancestors) {
        res
          .status(400)
          .send(`unable to get mempool ancestors for txid ${txid}`);
        return;
      }
    ...
    try {
      if (typeof blockHash !== 'string' || blockHash.length !== 64) {
        res
          .status(400)
          .send(
            `invalid param blockHash ${blockHash}. must be a string of 64 char`
          );
        return;
      }
      if (typeof verbosity !== 'string') {
        res
          .status(400)
          .send(
            `invalid param verbosity ${verbosity}. must be a string representing an integer`
          );
        return;
      }
      ...
      if (typeof verbosityNumber !== 'number') {
        res
          .status(400)
          .send(
            `invalid param verbosity ${verbosity}. must be a valid integer`
          );
        return;
      }
    ...
    try {
      if (typeof blockHeight !== 'string') {
        res
          .status(400)
          .send(
            `invalid param blockHeight ${blockHeight}, must be a string representing an integer`
          );
        return;
      }
      ...
      if (typeof blockHeightNumber !== 'number') {
        res
          .status(400)
          .send(
            `invalid param blockHeight ${blockHeight}. must be a valid integer`
          );
        return;
      }
      ...
      if (!block) {
        res
          .status(400)
          .send(
            `unable to get block hash for block height ${blockHeightNumber}`
          );
        return;
      }
      res.status(200).send(block);
